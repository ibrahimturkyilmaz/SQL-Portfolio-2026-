<div align="center">

# 🛒 Retail & Supply Chain Analytics Portfolio

**Perakende, CRM ve Üretim süreçlerine odaklanan SQL & Veri Analitiği projeleri.**

<p>
  <b>SuperStore</b> • <b>Online Retail</b> • <b>AdventureWorks</b>
</p>

</div>

---

## 👨‍💻 Proje Sahibi
**İbrahim Türkyılmaz** - Endüstri Mühendisliği Öğrencisi

---
> **📉 Analiz Örneği: Zarar Raporu**
> Aşağıdaki grafik, veritabanından çekilen verilerle oluşturulan bölgesel zarar analizini göstermektedir.
>
> ![SuperStore Zarar Raporu](./img/1-SuperStoreDataset-zarar-raporu.png)
>
> *Bu analizde görüldüğü üzere, belirli alt kategorilerde (Tables vb.) negatif kârlılık tespit edilmiştir.*

> **👥 Analiz Örneği: Müşteri Segmentasyonu**
> RFM skorlarına göre müşterilerin sadakat gruplarına ayrılması.
>
> ![Müşteri Segmentasyonu](./img/1-online-retail2-müşteri-segmentleme.png)

> **🏭 Analiz Örneği: Ürün Segmentasyonu & Tedarikçi Performansı**
>
> | Ürün Segmentasyonu (ABC) | Tedarikçi Performansı |
> | :---: | :---: |
> | ![Ürün Segmentasyonu](./img/1-adventureworks-ürün-segmentleme.png) | ![Tedarikçi Performansı](./img/2-adventureworks-tedarikci-performans-yorumu.png) |



## 📂 Projeler ve İş Senaryoları

### 1️⃣ Finansal & Satış Analizi (SuperStore Dataset) 📊
*Veri okuryazarlığı ve temel raporlama yetkinlikleri.*
* **Kârlılık Analizi:** Zarar eden bölgelerin ve ürünlerin tespiti (Loss Prevention).
* **KPI Raporlama:** Bölgesel satış performansları ve trend analizleri.

### 2️⃣ CRM & Müşteri Analitiği (Online Retail II) 👥
*Müşteri davranışlarını anlamak ve segmentasyon.*
* **RFM Analizi:** Müşterilerin Recency, Frequency, Monetary skorlarına göre segmentasyonu.
* **Cohort Analizi:** Müşteri tutundurma (Retention) oranlarının zaman bazlı takibi.
* **Market Basket Analysis:** Birlikte satın alınan ürünlerin tespiti (Cross-Sell).

### 3️⃣ Supply Chain & Üretim Analitiği (AdventureWorks 2019) 🏭
*Endüstri Mühendisliği problemlerine ileri seviye SQL çözümleri.*
* **ABC Stok Analizi:** Envanter yönetiminde Pareto (80/20) prensibinin uygulanması.
* **Vendor Rating:** Tedarikçilerin termin süresi ve kalite (fire oranı) performanslarının ölçülmesi.
* **BOM (Bill of Materials):** Recursive CTE kullanılarak ürün ağaçlarının alt parçalarına inilmesi.

---

## 🛠 Teknik Yetkinlikler

| Alan | Uygulanan Teknikler |
| :--- | :--- |
| **Database** | İlişkisel Veritabanı Yönetimi, T-SQL |
| **Advanced SQL** | Window Functions, CTE, Recursive Queries |
| **Engineering** | Veri Temizliği (Cleaning) & Manipülasyonu |
| **Strategy** | Pareto (ABC), RFM, Stok Optimizasyonu |

---
<div align="center">
  <i>Bu projeler, gerçek hayat perakende ve üretim senaryoları simüle edilerek hazırlanmıştır.</i>
</div>
